    ...
    def approximative_likelihood_fractal(self,cluster_idx,layer_centroids,sparse_mask,sparse_mask_z,sparse_mask_w,first_layer=False):
        ...
        if not self.scaling:
            if first_layer:
                ...
                sum_cl_idx_i=torch_sparse.spmm(sparse_mask_z._indices(), sparse_mask_z._values(), sparse_mask_z.shape[0], sparse_mask_z.shape[1], torch.exp(self.gamma).unsqueeze(-1))

                #sum_cl_idx_j=torch.sparse.mm(sparse_mask_w,torch.exp(self.alpha).unsqueeze(-1))
                sum_cl_idx_j=torch_sparse.spmm(sparse_mask_w._indices(), sparse_mask_w._values(), sparse_mask_w.shape[0], sparse_mask_w.shape[1], torch.exp(self.alpha).unsqueeze(-1))

                k_distance=self.pairwise_squared_distance_trick(layer_centroids, epsilon=1e-16)**0.5
                dist_exp=torch.exp(-k_distance)
                ...
                sum_cl_idx_i=torch_sparse.spmm(sparse_mask_z._indices(), sparse_mask_z._values(), sparse_mask_z.shape[0], sparse_mask_z.shape[1], torch.exp(self.gamma[self.mask_split_z]).unsqueeze(-1))

                #sum_cl_idx_j=torch.sparse.mm(sparse_mask_w,torch.exp(self.alpha[self.mask_split_w]).unsqueeze(-1))                   
                sum_cl_idx_j=torch_sparse.spmm(sparse_mask_w._indices(), sparse_mask_w._values(), sparse_mask_w.shape[0], sparse_mask_w.shape[1], torch.exp(self.alpha[self.mask_split_w]).unsqueeze(-1))

                k_distance=self.pdist(layer_centroids.view(-1,2,layer_centroids.shape[-1])[:,0,:],layer_centroids.view(-1,2,layer_centroids.shape[-1])[:,1,:])
